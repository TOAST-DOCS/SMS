...
3. 수신번호는 국내 번호 또는 국가 코드가 포함된 번호 모두 제공합니다. <br/>
예)01012345678, 821012345678
4. 첨부파일 업로드(최대 2개, 1개당 300K 이하)합니다.(파일명 45자 이내, 확장자는 jpg,jpeg(소문자))
...
### 중복 발송 차단 기능
1. 발송 설정 화면에서 중복 발송 차단 기능을 설정할 수 있습니다.
2. 중복 발송 차단 설정 및 시간을 지정한 경우 동일한 메세지로 발송 시 차단됩니다.
3. 동일 메세지 판단 기준은 다음과 같습니다.
    * 일반/대량/태그 발송 시 중복 체크는 별도로 체크합니다.(일반 발송과 동일한 내용의 대량 발송 시 중복 체크 안됨)
    * SMS/AUTH : 발신번호/수신번호/내용
    * LMS : 발신번호/수신번호/제목/내용
    * MMS : 발신번호/수신번호/제목/내용/첨부파일
        * 첨부 파일은 파일명이 아닌 업로드된 파일 기준입니다.
        * 콘솔 화면에서는 요청 하나에 여러 개의 수신자가 있는 경우 중복 체크가 정상적으로 진행됩니다.
        * API에서는 미리 업로드된 첨부파일ID(fileId) 기준입니다.

![[그림 37] 중복 발송 차단 설정 화면 ](http://static.toastoven.net/prod_sms/image_34.png)
