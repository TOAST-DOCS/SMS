...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (reference note not present in ko outline) -->
#### ※ For reference
...
- Before using the international SMS sending feature, make sure to check [[International SMS Sending Policy]](./international-sending-policy).
- If you do not want to use the international SMS sending feature, set it to unused to prevent accidents caused by international SMS volume pumping.
- Manage countries allowed to send
    - Only the specified major countries are enabled for sending during the initial setup. You can manage whether to ship to each country through the **Sending allowed countries settings > Select allowed countries** button.
- Auto blocking monthly limit and threshold notification
    ...
    - The **Auto blocking monthly limit** is a subsidiary feature and detection is not reflected in real time. NHN Cloud is not responsible for any errors in the subsidiary features, so please use with caution.
    - If you set the **Monthly limit threshold notification** to **Enable**, notification emails will be sent to all project members when 70%, 90%, and 100% of the value set for **Auto blocking monthly limit** is reached.
- Conversion rate-based sending blocking and notification
    - If you set **Conversion rate-based sending blocking and notification** to **Enable**, the feature is activated by country, and when blocking occurs, notification emails are sent to all project members.
    - Conversion rate-based sending blocking countries apply only to countries set through the **Conversion rate-based sending blocking country settings > Select blocking countries** button.
    ...
    - Conversion rate-based blocking rules
        - **Conversion rate** is the threshold setting for the criteria where conversion rate-based blocking occurs.
            - Blocking occurs when the calculated conversion rate is below the conversion rate threshold setting.
            - Conversion rate can be specified from 1% to 100%.
        - **Minimum count** is the threshold setting for the number of conversion rate collection request sends required for conversion rate-based blocking to operate.
            - Conversion rate-based blocking operates only after sending the minimum count or more conversion rate collection requests.
            - Minimum count can be specified from 1 to 10,000.
        - **Time range** is the time range setting for calculating conversion rate.
            - Conversion rate is calculated based on the number of conversion rate collection request sends and conversions within the time range from the send request time.
            - Time range can be specified from 1 hour to 168 hours (7 days).
- Manage daily sending volume limit settings by country
    - You can set daily sending volume for specific countries and configure by country so that no more sends occur when the sending volume is exceeded.
    - The daily sending volume standard by country is based on 0:00 Korean time (KST).
    - If blocked by this feature, you can unblock by setting a higher daily limit by country.
    - Daily sending volume limit countries by country apply only to countries set through the **Daily sending volume limit settings by country > Select limit countries** button.
    - The daily sending volume limit feature by country is a subsidiary feature and detection is not reflected in real time. NHN Cloud is not responsible for any errors in the subsidiary features, so please use with caution.
