...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/sender/sms' \
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/sender/mms' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "title": "{title}",
    "body": "{body}",
...
* When searching by registration date and delivery date simultaneously, delivery date is ignored.
...
| recipientNo          | String  | 20         | Optional | Recipient number                                                               |
| templateId           | String  | 50         | Optional | Template number                                                                |
| msgStatus            | String  | 1          | Optional | Message status code (0: Failed, 1: Requested, 2: Processing, 3: Success, 4: Reservation canceled, 5: Duplicate failure) |
...
| countryCode          | String  | -          | Optional | Country code [[Transmittable countries](./international-sending-policy/#_5)]  |
| pageNum              | Integer | -          | Optional | Page number (default: 1)                                                      |
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/sender/auth/sms' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "body": "인증 테스트",
    "sendNo": "15446859",
...
[Request Body]
Same as SMS sending above.
[[See Request Body](./api-guide/#sms_2)]

<span style="color:red">However, the message body must include the required advertising text.</span>

You can check the 080 number in the **080 Opt Out Settings** tab in the console.

The rules for required advertising text are as follows:
- Opening text: `(Ad)`
- Closing text: `Toll-free opt-out {080 opt-out number}` or `Toll-free opt-out {080 opt-out number}`
  - The text may include spaces.
  - The 080 opt-out number may include '-' between digits.
...
Toll-free opt-out 080-XXX-XXXX
```
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/sender/ad-sms' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "body": "(광고) Test\n [무료 수신 거부]0808880327",
    "sendNo": "15446859",
...
<span style="color:red">However, the body must include mandatory phrases for advertising.</span>

The 080 number can be checked in the **080 Opt-out Settings** tab on the console.

The rules for mandatory advertising phrases are as follows:
- Opening phrase: `(Ad)`
- Closing phrase: `Toll-free Opt-out {080 opt-out number}` or `Toll-free Block {080 opt-out number}` (These phrases may include spaces.)
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/sender/ad-mms' \
-H 'Content-Type: application/json;charset=UTF-8'
-d '{
    "title": "{제목}",
    "body": "(광고) 테스트\n [무료 수신 거부]0808880327",
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/tag-sender/sms' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "body": "본문",
    "sendNo": "15446859",
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/tag-sender/mms' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "title": "제목",
    "body": "본문",
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/attachfile/binaryUpload' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (extra Request heading under Upload Attached Files) -->
<a id="request-18"></a>
...
| Http method | URL                                                                               |
|-------------|-----------------------------------------------------------------------------------|
| POST        | https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/{appKey}/attachfile/binaryUpload |
...
```
curl -X PUT \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/categories/'"${C_ID}" \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (missing L4 heading for template sending example) -->
#### Example
...
```
curl -X PUT \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/reservations/cancel' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "reservationList": [{
            "requestId": "1",
...
| body.data.canceledCount  | Integer  | Number of successful cancels |

<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (missing Response heading for Cancel Scheduled Delivery) -->
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/sender/download-reservations' \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "sendType": "1",
...
```
curl -X POST \
'https://sms.api.nhncloudservice.com/sms/v2.4/appKeys/'"${APP_KEY}"'/uids/'"${USER_ID}/phone-numbers" \
-H 'Content-Type: application/json;charset=UTF-8' \
-d '{
    "phoneNumber": "0100000000"
}'
