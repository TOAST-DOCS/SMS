...
+ 국제 SMS 메시지를 정상적으로 발송하기 위해 국가별 정책에 따라 최대 12자의 글자가 메시지 발송 내용에 추가되어 발송될 수 있으며 해당 글자 수는 과금 글자수에 포함됩니다.
...
+ 메시지 concat 시 메시지를 연결하기 위한 헤더가 처리되는 과정에서 글자수가 일부 줄어듭니다.
+ 글자수 및 concat 기준은 국제 SMS 표준 규격을 따릅니다.
+ concat 하여 발송한 메시지일지라도 통신사 및 단말기 정책에 따라 긴 메시지가 아닌 단문 메시지 여러 건의 형태로 단말기에 수신될 수 있습니다.
