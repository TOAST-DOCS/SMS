...
## 메시지 수신 결과 타임 아웃 정책
+ 단말기 및 통신 상태에 따라 메시지 수신 결과 업데이트가 지연될수 있습니다.
+ 메시지 수신 결과 지연 시 통신사에서 정의한 재전송 정책에 따라 발송을 시도합니다. 
+ 재전송 정책은 다음과 같습니다.

| 발송 타입 | 타임 아웃 시간 | 타임 아웃 이후 |
|---|---|---|
| SMS | 25 시간 | 재시도 하지 않음. 수신 실패 결과 업데이트 (결과 코드 2000) |
| LMS | 80 시간 | 재시도 하지 않음. 수신 실패 결과 업데이트 (결과 코드 2000) |
| MMS | 80 시간 | 재시도 하지 않음. 수신 실패 결과 업데이트 (결과 코드 2000) |