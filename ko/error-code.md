...
| BUFFERED | 메시지가 전송 전 대기중인 상태 |
| EXPIRED | 이동통신사의 재전송 정책으로 인해 메시지가 전송 보류되고 만료 시간 내 전송할 수 없는 상태 |
| FAILED | 메시지 전송 실패 |
| REJECTED | 이동통신사가 메시지 전송을 거부한 상태 |
| UNKNOWN | 유용한 정보를 찾을 수 없음 |
...
| 4 | 사용자에 의해 차단된 번호 | 영구적인 오류로 해당 번호를 반드시 데이터베이스에서 삭제하고 네트워크 운영자에게 문의 후 차단을 해제해야 함 |
| 5 | 이동성 오류 | 번호 이동성과 관련된 문제가 있는 경우 네트워크 운영자에게 문의하여 해결해야 함 |
| 6 | 스팸 방지 거부 | 메시지가 통신사의 스팸 방지 필터에 의해 차단됨 |
| 7 | 단말기 사용 중 | 메시지 전송 시 단말기 사용 불가 상태 - 다시 시도하세요 |
| 8 | 네트워크 오류 | 네트워크 오류로 인해 메시지 전송 실패 - 다시 시도하세요 |
| 9 | 잘못된 번호 | 사용자가 특정 서비스로부터 메시지 수신 거부를 특별히 요청한 경우 |
| 11 | 라우팅 불가 | NHN Cloud에서 메시지 전송을 위한 적절한 경로를 찾지 못함 - 고객센터에 문의하세요 |
| 12 | 연결할 수 없는 목적지 | 해당 번호로 연결되는 경로를 찾을 수 없음 - 수신자 번호를 확인하세요 |
| 13 | 수신자 나이 제한 | 나이 제한으로 수신인의 메시지 수신 불가 |
| 14 | 통신사에 의해 차단된 번호 | 수신인의 요금제에서 SMS 서비스 사용할 수 있도록 통신사에 문의해야 함 |
| 16 | 게이트웨이 할당량 초과 | 기간당 허용된 요청 횟수를 초과로 메시지 전송 실패. 해당 오류는 미국과 프랑스에 등록된 계정만 해당됩니다 |
| 20 | 부정 행위 방지 트래픽 규칙 | 메시지가 트래픽 펌핑에 의해 거부됨 - 고객센터에 문의하세요 |
| 21 | 비정상 연속적 발신 감지 | 고밀도 수신번호 범위 임계치를 초과했습니다 |
| 22 | 비정상 트래픽 급증 감지 | 상대적 증가 임계치를 초과했습니다 |
| 39 | 목적지가 미국인 잘못된 발신자 주소 | Sender ID 문제(sendNo problem)로 미국으로 메시지 전송 실패 - 고객센터에 문의하세요 |
| 51 | 헤더 필터 | Sender ID 문제(sendNo problem)로 미국으로 메시지 전송 실패 - 고객센터에 문의하세요 |
| 53 | 동의 필터 | 동의하지 않아 메시지 전송 실패 |
| 54 | 규제 오류 | 예상치 못한 규제 관련 오류 - 고객센터에 문의하세요 |
| 99 | 일반 오류 | 일반적으로 경로 오류를 의미함 - 고객센터에 문의하세요 |
| 1000 | 기타 오류 | 그 외 오류 |
