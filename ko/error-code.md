...
2) G/W: G/W가 메시지 수신 후 전달하는 결과 코드이다.

3) Server : Server가 메시지 전송 요청에 대해 처리한 오류 코드이다.
...
| 이통사 | 3017 | 실패 | 발신 번호(=회신 번호) 변작 방지 서비스에 의거한 번호 형식 오류 |
| 이통사 | 3018 | 실패 | 발신 번호(=회신 번호) 변작 방지 서비스에 가입된 휴대폰 개인 가입자 번호 |
| 이통사 | 3019 | 실패 | 발신 번호(=회신 번호) 인포뱅크에 번호 사전 등록제를 통해 등록되지 않은 번호 |
| G/W | 1001 | 실패 | Server Busy |
| G/W | 1002 | 실패 | 수신 번호 형식 오류 |
| G/W | 1002 | 실패 | 회신 번호 형식 오류 |
| G/W | 1019 | 실패 | TTL 초과 |
| Server | E911 | 실패 | 첨부파일 확장자가 없는 경우 |
| Server | E913 | 실패 | 첨부파일 크기가 0인 경우 |
| Server | E915 | 실패 | 중복메시지 |
| Server | E919 | 실패 | 발송 제한 시간인 경우, 메시지 재발송 처리가 금지된 경우 |
| Server | E999 | 실패 | 기타 오류 |