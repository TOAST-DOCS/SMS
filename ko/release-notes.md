...
### 2020. 08. 25
### 기능 개선/변경
* [Console] 대량 발송 엑셀 유효성 검사 강화
    * 수신자 번호를 입력하지 않은 경우 유효성 검사에 의해 실패 처리됩니다.
* [Console] **발신 번호 등록** 탭의 업로드 파일 다운로드 불가능한 현상 수정
    * **발신 번호 관리 > 발신 번호 등록** 탭에서 서류 인증으로 업로드 후 해당 파일 다운로드가 안 되는 문제를 수정하였습니다. 
* [Console/API] 첨부파일 이미지 포맷 유효성 검사 강화
    * MMS 발송을 위한 첨부파일 업로드 시, 이미지 포맷이 jpg가 아닌 경우 실패 처리됩니다.
* [Console] 백업 설정 유효성 검사 강화
    * **발송 설정** 탭의 백업 설정에서 파일 저장 경로 앞/뒤에 '/'를 붙인 경우 유효성 검사에서 실패 처리됩니다. 
