...
* 전환율 기반 발송 차단 및 알림
    ...
    * 전환율 기반 차단 규칙
        * **전환율**은 전환율 기반 차단이 발생하는 기준 임계치 설정입니다.
            - 계산된 전환율이 전환율 임계치 설정값 이하인 경우 차단이 발생합니다.
            - 전환율은 1%부터 100%까지 지정 가능합니다.
        * **최소 건수**는 전환율 기반 차단이 동작하기 위한 전환율 수집 요청 발송 건수의 임계치 설정입니다.
            - 최소 건수 이상의 전환율 수집 요청 발송건을 발송해야 전환율 기반 차단이 발생합니다. 
            - 최소 건수는 1건부터 10,000건까지 지정 가능합니다.
        * **시간 범위**는 전환율을 계산하는 시간 범위 설정입니다. 
            - 발송 요청 시간부터 해당 시간 범위 내의 전환율 수집 요청 발송 건수, 전환 건수를 기준으로 전환율을 계산합니다. 
            - 시간 범위는 1시간부터 168시간(7일)까지 지정 가능합니다.
