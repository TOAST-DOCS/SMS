...
- 개인 회원은 서비스를 이용할 수 없습니다(2023년 12월 15일 기준으로 개인 회원 서비스 이용 정책이 변경되었습니다).
...
- 발신 번호 차단으로 문자 전송에 실패한 경우에는 '번호 도용 문자 차단 서비스'를 확인하세요. [[가이드 바로 가기]](./sending-policy/#fraud-number)
- 전송 결과는 성공이지만 문자를 수신하지 못하는 경우는 '통신사 스팸 차단 서비스'를 확인하세요. [[가이드 바로 가기]](./sending-policy/#spam-number)
...
!!! danger "주의"
템플릿 치환 기능을 사용할 경우, 제목 또는 내용에 '##key##'와 같이 치환 키를 ##으로 묶어 입력하세요.
...
- recipient\_no 열이 존재하지 않는 경우: recipient\_no 열은 수신 번호를 입력하는 열로 필수로 입력해야 합니다.
- 파일에 입력된 데이터가 없는 경우
...
- 입력하는 업체명은 080 수신 거부 번호로 전화를 걸 때, 안내되는 업체명입니다.
...
- **요청 ID** 또는 **수신 번호** 클릭 시 상세 조회 팝업이 노출됩니다.
...
- 조회: 상단의 조회 폼에서 대량 SMS 발송 예약 건을 조회할 수 있습니다. 조회 건의 리스트 행을 선택하면 하단의 조회 폼에서 수신 번호와 발송 정보(발송 내용, 발송 결과)를 확인할 수 있습니다.
...
- 발송 준비 완료: 수신자 파일 데이터를 모두 로드하여 SMS 발송 준비가 완료된 상태입니다. 예약 건(리스트의 행)을 선택하면 하단의 리스트에서 수신 번호와 발송 내용을 확인할 수 있습니다.
...
- 자동 차단 월 제한 건수 및 임계치 알림
    ...
    - **자동 차단 월 제한 건수**는 보조 기능으로 탐지가 실시간으로 반영되지 않습니다. 보조 기능의 일부 오차에 대해 NHN Cloud는 책임을 지지 않으니 주의해서 사용하세요.
    - **월 제한 건수 임계치 알림**을 **사용**으로 설정하면 **자동 차단 월 제한 건수**로 설정한 값의 70%, 90%, 100% 도달 시 프로젝트 멤버 전체에게 알림 메일이 발송됩니다.
- 전환율 기반 발송 차단 및 알림
    - **전환율 기반 발송 차단 및 알림**을 **사용**으로 설정하면 국가별로 기능이 활성화되며, 차단 발생 시 프로젝트 멤버 전체에게 알림 메일이 발송됩니다.
    - 전환율 기반 발송 차단 국가는 **전환율 기반 발송 차단 국가 설정 > 차단 국가 선택** 버튼을 통해 설정한 국가에 대해서만 적용됩니다.
    - 전환율에 의해 차단된 국가 옆에 **Blocked** 버튼이 노출됩니다. 해당 버튼을 클릭하면 차단을 해제할 수 있습니다.
    - 전환율 기반 차단 규칙
        - **전환율**은 전환율 기반 차단이 발생하는 기준 임계치 설정입니다.
            - 계산된 전환율이 전환율 임계치 설정값 이하인 경우 차단됩니다.
            - 전환율은 1%부터 100%까지 지정할 수 있습니다.
        - **최소 건수**는 전환율 기반 차단이 동작하기 위한 전환율 수집 요청 발송 건수의 임계치 설정입니다.
            - 최소 건수 이상의 전환율 수집 요청 건을 발송해야 전환율 기반 차단이 동작합니다.
            - 최소 건수는 1건부터 10,000건까지 지정할 수 있습니다.
        - **시간 범위**는 전환율을 계산하는 시간 범위 설정입니다.
            - 발송 요청 시간부터 해당 시간 범위 내의 전환율 수집 요청 발송 건수, 전환 건수를 기준으로 전환율을 계산합니다.
            - 시간 범위는 1시간부터 168시간(7일)까지 지정할 수 있습니다.
- 국가별 1일 발송량 제한 설정 관리
    - 특정 국가별로 1일 발송량을 설정할 수 있으며 발송량 초과시 더 이상 발송되지 않게 국가별로 설정할 수 있습니다.
    - 국가별 1일 발송량 기준은 한국 시간 기준(KST) 0시입니다.
    - 해당 기능에 의해 차단된 경우 국가별 1일 한도를 더 높게 설정하여 차단을 해제할 수 있습니다.
    - 국가별 1일 발송량 제한 국가는 **국가별 1일 발송량 제한 설정 > 제한 국가 선택** 버튼을 통해 설정한 국가에 대해서만 적용됩니다.
    - 국가별 1일 발송량 제한 기능은 보조 기능으로 탐지가 실시간으로 반영되지 않습니다. 보조 기능의 일부 오차에 대해 NHN Cloud는 책임을 지지 않으니 주의해서 사용하세요.
...
- 다음 시간 기준으로 통계가 수집됩니다.
    ...
    - 발송 실패: 실패 응답이 발생한 시간
    - 수신: 발송된 메시지의 수신 시간
    - 발송 건수: 단건 혹은 Concatenated message(연결) 기능을 통해 발송된 메시지의 발송 시간
    - 전환 대기: 전환율 수집 요청 발송 건의 메시지 수신 시간
...
4. 태그를 선택한 후 문자를 발송합니다.
    - **일반 발송** 탭에서 전화번호 대신 **태그 발송**을 선택하여 태그를 등록합니다.
...
- 다만, 템플릿에 등록된 첨부 파일 및 증빙 서류(통신서비스 이용증명원)는 서비스를 이용하는 동안 보관됩니다.
