...
+ SMS 물량 펌핑 발생 방지를 위해 메시지를 보낼 의도가 없는 국가 코드로의 발송을 막아두면 SMS 물량 펌핑 가능성이 낮아집니다. 또한 유사하거나 동일한 번호로의 OTP 인증 요청 허용 횟수와 X 초당 메시지 발송 속도를 적절한 수준으로 제한하면 피해 규모를 낮출 수 있습니다. 
+ SMS 물량 펌핑 현상 발생 시 해당 AppKey에 대하여 사전 고지 없이 일부 혹은 전체 국제 SMS 물량을 발송 차단할 수 있습니다. 어뷰징 발생 건 및 어뷰징 발생으로 차단된 건에 대하여 NHNCloud는 일체의 책임을 지지 않습니다.
