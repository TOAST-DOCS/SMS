...
### 전환율 기반 차단 규칙 설정 및 차단 해제
+ 콘솔 **발송 설정 > 국제 SMS > 전환율 기반 발송 차단 국가 설정** 메뉴에서 전환율 기반 차단 규칙 설정과 차단 해제가 가능합니다.
+ 전환율 기반 차단 규칙 설정에 따라 전환율 계산, 전환율 기반 차단이 국가별로 발생합니다.
+ 전환율 기반 차단 규칙 설정에 따라 차단된 국가는 콘솔에서 확인 가능하며, 차단 버튼 선택 시 해제가 가능합니다.
    + 차단 해제 시 재 차단이 발생하지 않도록 전환율 차단 해제를 한 시점이 설정한 차단 규칙의 시간 범위보다 이후인 경우 차단 해제를 한 시점부터 전환율이 계산됩니다.
