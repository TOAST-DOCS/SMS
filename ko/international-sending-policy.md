...
+ 국제 SMS는 현지 이동통신사업자로 전송되고 DLR을 반환합니다.
+ 각 DLR은 메시지 상태와 결과 코드를 제공하며, 이를 통해 특정 SMS의 전송 상태를 알 수 있으나 정확성을 절대적으로 보장할 수는 없으며 이동통신사업자 및 단말기 상황에 따라 메시지 상태와 결과 코드가 값이 NULL일 수 있습니다.
