...
+ 회원가입 인증 번호 요청 등의 페이지에서 봇 혹은 어뷰저가 메시지 발송을 대량 요청합니다.
+ 대부분의 봇 혹은 어뷰저는 인증 요청 후 실제 인증은 하지 않습니다. 어뷰징이 발생하면 인증번호 요청은 늘지만, 인증을 수행하고 전환되는 비율은 줄어듭니다.
+ 유의사항
    + 국제 SMS 물량 펌핑 현상 발생 시 해당 프로젝트에 대하여 사전 고지 없이 일부 혹은 전체 국제 SMS 물량을 발송 차단할 수 있습니다. 
    + 어뷰징 발생이나 이로 인해 차단이 발생하여 피해가 발생하더라도 NHN Cloud는 일체의 책임을 지지 않으니, 기밀정보 유출 및 어뷰징에 대해 주의하시기 바랍니다.
+ 권장 조치 사항
    + 발송 허용 국가 코드 관리를 통해 메시지를 보낼 의도가 없는 국가로의 발송을 제한합니다.
    + 월별로 발송 가능한 건수를 적절한 수준으로 제한합니다.
    + 전환율에 의한 차단 국가 설정을 통해 전환율이 낮은 국가로의 발송이 차단되도록 설정합니다.
...
## 국제 SMS 전환율에 의한 차단
### 전환과 전환율
+ NHN Cloud는 사용자의 전환율 수집 요청과 전환 API 호출을 통해 국제 SMS 발송의 신뢰성을 높이고 어뷰징에 대한 차단을 강화합니다.
+ 전환이란 일반적으로 URL 클릭, 인증 코드 입력과 같은 특정 사용자 작업의 완료 여부를 의미합니다.
    + 수신자에게 인증 번호를 요청한 후 해당 인증 번호가 정상적으로 인증에 이용이 되었다면 해당 메시지는 전환된 것으로 간주합니다. 
+ 국제 SMS 발송 요청의 전환율 수집 요청 필드를 통해 해당 발송건을 전환율 수집 대상으로 설정합니다.
    + 자세한 사항은 [[API v3.0 가이드](./api-guide/#sms_1)]를 참고 바랍니다.
+ 전환율 수집 대상에 포함된 발송 건이 정상적으로 전환되었다 판단되는 경우 전환 API 호출을 통해 NHN Cloud에 전환 여부를 알릴 수 있습니다.
    + 전환율 수집 요청한 발송 건에 대해 전환 API 호출을 통해 전환을 시도해야 합니다.
    + 전환율 수집 요청을 하였더라도 정상적으로 발송이 완료된 발송건에 대해 전환 API 호출을 통해 전환을 시도해야 합니다.
    + 자세한 사항은 [[API v3.0 가이드](./api-guide/#sms_1)]를 참고 바랍니다.
+ 전환율 수집 대상 메시지의 발송 성공건 중 정상적으로 전환된 발송건의 비율에 따라 해당 국가의 전환율을 계산합니다.
 
### 전환율에 의한 차단 기능 사용 및 전환율 관리
+ 전환율에 의한 차단 기능을 활성화하여 전환율에 의한 차단 기능을 활성화 할 수 있습니다.
    + 전환율에 의한 차단 기능은 콘솔 발송 설정 > 국제 SMS 발송 설정에서 설정할 수 있습니다.
    + 전환율 콘솔에 대한 자세한 사항은 [[콘솔 사용 가이드](./console-guide/#sms_8)]를 참고 바랍니다.
+ 전환율에 의한 차단 기능을 활성화하면 전환율이 낮은 국가로의 발송이 차단됩니다.
    + '전환율에 의한 차단 국가 설정'을 통해 전환율 차단 기능을 설정한 국가에서 최근 24시간동안 50건 이상의 메시지가 발송되었고, 전환율이 50% 이하가 될 경우 해당 국가로의 발송이 자동 차단됩니다.
+ 콘솔 발송 설정 > 국제 SMS 발송 설정 > 전환율에 의한 차단 국가 설정에서 국가별 전환율에 의한 차단 해제가 가능합니다.
    + 전환율에 의한 차단을 해제하더라도 국가별 전환율 지표는 초기화되지 않습니다.
    + 차단 해제 후 발송을 진행하는 경우 해당 시점의 전환율에 따라 다시 차단이 발생할 수 있습니다.
...
| 우간다 | Uganda | 256 |
| 우크라이나 | Ukraine | 380 |
| 아랍에미리트연합 | United Arab Emirates | 971 |
