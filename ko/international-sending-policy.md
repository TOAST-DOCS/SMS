...
+ 메시지 Concatenated message 시 메시지를 연결하기 위한 헤더가 처리되는 과정에서 글자 수가 일부 줄어듭니다.
+ GSM-7bit 중 '{' 및 ']'와 같은 일부 확장 문자는 인코딩 과정에서 두 글자 수를 사용합니다.
+ 글자 수 및 Concatenated message 기준은 국제 SMS 표준 규격을 따릅니다.
+ Concatenated message 하여 발송한 메시지일지라도 통신사 및 단말기 정책에 따라 긴 메시지가 아닌 단문 메시지 여러 건의 형태로 단말기에 수신될 수 있습니다.
