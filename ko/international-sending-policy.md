...
## 전환율 관리
+ 전환율에 의한 차단 기능을 활성화하여 전환율을 관리할 수 있습니다.
+ 국제 SMS 발송 시 해당 메시지를 전환율에 포함시킬 수 있습니다. 자세한 사항은 [[API v3.0 가이드](./api-guide/#sms_1)]를 참고 바랍니다.
+ 발송한 국제 SMS가 정상적인 메시지로 판단되는 경우 전환 API를 통해 NHN Cloud에 발송의 신뢰성을 알릴 수 있습니다. 전환 API의 성공을 통해 NHN Cloud는 해당 국가의 전환율을 계산합니다.
    + 전환 API에 대한 자세한 사항은 [[API v3.0 가이드](./api-guide/#sms_1)]를 참고 바랍니다.
+ '전환율에 의한 차단 국가 설정'을 통해 전환율 차단 기능을 설정한 국가에서 최근 24시간동안 50건 이상의 메시지가 발송되었고, 인증 전환율이 50% 이하가 될 경우 해당 국가로의 발송이 자동 차단됩니다.
+ 전환율 차단은 콘솔 발송 설정 > 국제 SMS 발송 설정 > 전환율에 의한 차단 국가 설정에서 확인할 수 있습니다.
    + 전환율 콘솔에 대한 자세한 사항은 [[콘솔 사용 가이드](./console-guide/#sms_8)]를 참고 바랍니다.
+ 전환율 차단 해제를 진행하여도 최근 24시간의 전환율 지표가 회복되지 않는 경우 재 차단이 발생합니다.
