...
+ 유의사항
    + 국제 SMS 물량 펌핑 현상 발생 시 해당 프로젝트에 대하여 사전 고지 없이 일부 혹은 전체 국제 SMS 물량을 발송 차단할 수 있습니다. 어뷰징 발생이나 이로 인해 차단이 발생하여 생기는 피해에 대해 NHN Cloud는 일체의 책임을 지지 않습니다.
+ 권장 조치 사항
...
+ 국제 SMS 발송 시 전환율 수집 요청 필드를 통해 해당 메시지를 전환율 수집 대상에 포함할 수 있습니다. 자세한 사항은 [[API v3.0 가이드](./api-guide/#sms_1)]를 참고 바랍니다.
+ 전환율 수집 대상에 포함된 발송 건이 정상적인 메시지로 판단되는 경우 전환 API 호출을 통해 NHN Cloud에 발송의 신뢰성을 알릴 수 있습니다. 자세한 사항은 [[API v3.0 가이드](./api-guide/#sms_1)]를 참고 바랍니다. 
+ 전환율 수집 대상 중 전환 API의 정상 호출을 통해 전환된 발송건의 비율에 따라 해당 국가의 전환율을 계산합니다.
