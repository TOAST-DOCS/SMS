...
+ SMS 물량 펌핑 발생 방지를 위해 메시지를 보낼 의도가 없는 국가 코드로의 발송을 막아두면 SMS 물량 펌핑 가능성이 낮아집니다. 또한 유사하거나 동일한 번호로의 OTP 인증 요청 허용 횟수와 X 초당 메시지 발송 속도를 적절한 수준으로 제한하면 피해를 규모를 낮출 수 있습니다. 
