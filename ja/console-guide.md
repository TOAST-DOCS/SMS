...
<table class="custom-table" style="text-align: center">
  ...
  <tbody>
      <tr>
          <td rowspan=3>事業者</td>
          <td rowspan=2>NHN Cloudアカウント</td>
          <td>事業者代表が認証する場合</td>
...
- 内容の標準規格
    - 韓国SMS 90byte(ハングル45文字、英字90文字)、韓国LMS 2,000byte(ハングル1,000文字、英字2,000文字)、韓国MMS 2,000byte(画像含むハングル1,000文字、英字2,000文字)
...
!!! danger "注意"
    テンプレート置換機能を使用する場合、件名または本文に'##key##'のように置換パラメータを##で囲んで入力してください。
...
- recipient_no列が存在しない場合：recipient_no列は受信番号を入力する列であり、必須で入力する必要があります。
- ファイルに入力されたデータがない場合
...
- 入力する企業名は、080受信拒否番号に電話をかけた際に案内される企業名です。
...
- **リクエストID**または**受信番号**をクリックすると、詳細照会ポップアップが表示されます。
...
* 自動ブロック月間制限件数及びしきい値通知
    ...
    * **月間制限件数しきい値通知**を**使用**に設定すると、**自動ブロック月間制限件数**で設定した値の70%、90%、100%に達するとプロジェクトメンバー全員に通知メールが送信されます。
...
- 国別の1日送信量制限設定の管理
    - 特定の国別に1日の送信量を設定でき、送信量を超過した場合はそれ以上送信されないように国別に設定できます。
    - 国別の1日送信量の基準は、韓国時間(KST)の0時です。
    - この機能によって制限された場合、国別の1日の限度をより高く設定することで制限を解除できます。
    - 国別の1日送信量制限の国は、**国別の1日送信量制限設定 > 制限国の選択**ボタンから設定した国に対してのみ適用されます。
    - 国別の1日送信量制限機能は補助機能であり、検知がリアルタイムで反映されるわけではありません。補助機能の一部誤差についてNHN Cloudは責任を負いかねますので、ご注意の上ご使用ください。

!!! danger "注意"
    世界的に国際SMSに対する不正利用の事例が増加しています。
    月の制限件数と送信国の指定については、必ず必要な分だけ設定することを推奨します。
...
1. UIDを登録します。
    ...
    - 詳細については、[UID管理](./console-guide/#uid-manage)をご参照ください。
2. タグを登録します。
    - **タグ管理**タブでタグを登録します。
    - 詳細については、[タグ管理](./console-guide/#tag-manage)をご参照ください。
3. タグにUIDを登録します。
    - **タグ管理**タブで、登録したタグにUIDを登録します。
4. タグを選択し、メッセージを送信します。
    - **一般送信**タブで電話番号の代わりに**タグ送信**を選択してタグを登録します。
