...
> SMSサービスを利用するには、[コンソール > SMS > 発信番号管理]で発信番号を事前に登録した後、使用できます(電気通信事業法)。
...
* 内容標準規格：国内SMS 90バイト（ハングル45文字、英語90文字） / 国内MMS 2,000バイト（ハングル1,000文字、英語2,000文字） / 国際SMS UCS-2 335文字 / 国際SMS GSM-7bit 765文字   
...
1. **送信タイプ**はSMSを選択します。
2. **発信番号**を選択します。(発信番号は**発信番号事前登録** > **発信番号登録および名義人認証**タブで追加できます。)
...
* 国際SMS送信機能を使用しない場合は、未使用に設定し、アビューズによる事故を未然に防止します。
* 初回使用設定時に指定された主要国のみ送信できるように設定されます。[許可する国選択]ボタンで国別送信の可否を管理します。
* 送信制限は月1,000件が基本で、最大10,000件まで適用可能です。10,000件を超える調整が必要な場合は、[1万件超過希望]ボタンからお問い合わせください。
* 「国際自動ブロック月間制限件数」は補助機能で、ブロックのタイミングが正確でない場合があります。補助機能の一部誤差について、NHN Cloudは一切の責任を負いません。
