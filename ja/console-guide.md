...
* 国際SMS送信機能を使用しない場合は未使用に設定して国際SMS物量ポンピング現象による事故を防止します。
* 送信許可国家管理
    * 最初の使用設定時に指定された主要国家のみ送信可能に設定されます。**送信許可国家設定 > 許可国家選択**ボタンを通じて国家別送信可否を管理します。
* 自動ブロック月制限件数及び閾値アラート
    * **自動ブロック月制限件数**は月1,000件が基本で、最大10,000件まで適用可能です。
    * **自動ブロック月制限件数**最大値である10,000件を超過した調整が必要な場合、**1万件超過希望**ボタンを通じてお問い合わせください。
    * **自動ブロック月制限件数**は補助機能として検知がリアルタイムで反映されません。補助機能の一部誤差についてNHN Cloudは責任を負いませんのでご注意してご利用ください。
    * **月制限件数閾値アラート**を**使用**に設定すると**自動ブロック月制限件数**で設定した値の70%、90%、100%到達時にプロジェクトメンバー全体にアラートメールが送信されます。
* 転換率ベース送信ブロック及びアラート
    * **転換率ベース送信ブロック及びアラート**を**使用**に設定すると国家別に機能が活性化され、ブロック発生時にプロジェクトメンバー全体にアラートメールが送信されます。
    * 転換率ベース送信ブロック国家は**転換率ベース送信ブロック国家設定 > ブロック国家選択**ボタンを通じて設定した国家に対してのみ適用されます。
    * 転換率によってブロックされた国家の隣に**Blocked**ボタンが表示されます。該当ボタンをクリックするとブロックを解除できます。
    * 転換率ベースブロック規則
        * **転換率**は転換率ベースブロックが発生する基準閾値設定です。
            - 計算された転換率が転換率閾値設定値以下の場合ブロックされます。
            - 転換率は1%から100%まで指定できます。
        * **最小件数**は転換率ベースブロックが動作するための転換率収集リクエスト送信件数の閾値設定です。
            - 最小件数以上の転換率収集リクエスト件を送信してこそ転換率ベースブロックが動作します。
            - 最小件数は1件から10,000件まで指定できます。
        * **時間範囲**は転換率を計算する時間範囲設定です。
            - 送信リクエスト時間から該当時間範囲内の転換率収集リクエスト送信件数、転換件数を基準に転換率を計算します。
            - 時間範囲は1時間から168時間（7日）まで指定できます。
* 国家別1日送信量制限設定管理
    * 特定国家別に1日送信量を設定でき、送信量超過時にそれ以上送信されないよう国家別に設定できます。
    * 国家別1日送信量基準は韓国時間基準（KST）0時です。
    * 該当機能によってブロックされた場合、国家別1日限度をより高く設定してブロックを解除できます。
    * 国家別1日送信量制限国家は**国家別1日送信量制限設定 > 制限国家選択**ボタンを通じて設定した国家に対してのみ適用されます。
    * 国家別1日送信量制限機能は補助機能として検知がリアルタイムで反映されません。補助機能の一部誤差についてNHN Cloudは責任を負いませんのでご注意してご利用ください。

> [注意]
世界的に国際SMSに対するアビューシング事例が増加しています。
月制限件数と送信国家指定については必ず必要な分だけ設定することを推奨します。
NHN Cloudはアビューシングで送信された国際SMSについて一切の責任を負いません。
