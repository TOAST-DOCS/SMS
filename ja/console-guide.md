...
* 送信許可国の管理
    * 最初の使用設定時に指定された主要国のみ送信できるように設定されます。**送信許可国管理 > 許可する国を選択**ボタンを通じて、国別の送信可否を管理します。
...
* コンバージョン率によるブロック国の設定とお知らせ
    * **コンバージョン率によるブロックと通知**を**使用**に設定すると、国別にコンバージョン率によるブロック機能が有効になり、ブロックが発生するとプロジェクトメンバー全員に通知メールが送信されます。
    * コンバージョン率による国別ブロックは、**コンバージョン率によるブロック国設定 > 許可する国を選択**ボタンで設定した国に対してのみ適用されます。
    * コンバージョン率によって国がブロックされた場合、ブロックされた国の横に'Blocked'ボタンが表示されます。このボタンをクリックすると、ブロックを解除できます。
...
* 次の時間基準で統計が収集されます。
    ...
    * 送信:通信事業者(ベンダー)に送信リクエストした時間
    * 送信失敗:失敗レスポンスが発生した時間
    * 受信:単件またはConcatenated message(接続)機能を介して送信されたメッセージの受信時間
    * 結果待機中:送信と同時に発生して数が増加、受信すると数が減少
