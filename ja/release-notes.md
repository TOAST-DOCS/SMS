...
---
* [Console] 国家別1日送信上限制限
  * 国際送信時に国家別で1日送信上限を制限できる機能が追加されました。
...
* [Console] メッセージ送信履歴バックアップ結果通知追加
  * メッセージ保管期間が過ぎたメッセージ送信履歴のバックアップ設定時に結果通知を受信できます。
...
#### 機能追加
* [API] 080受信拒否番号照会APIの追加
    * 080受信拒否番号を照会できるAPIが追加されました。
    * 詳細は[[API v3.0ガイド > 080受信拒否サービス](./api-guide/#080)]を参照してください。
...
#### 機能改善・変更
* [Console/API] 広告送信時における080受信拒否番号の有効性検証ロジックを緩和
  * 広告を送信する際に、080受信拒否番号の途中に「-」を挿入しても、正常に送信されるように変更されました。
* [ETC] 通知管理 - 月間送信量制限の通知を追加
  * 設定された月間送信量制限値の70%、90%、100%に到達するたびに、自動で通知を受け取ることができます。
...
#### バグ修正
* [API] 統計照会バグ修正
  * 統計照会時にデータ検索範囲を狭く設定した場合、データが2倍に照会されるバグが修正されました。
...
* [ETC] Cloud Monitoring指標照会追加
    * Cloud Monitoringサービスで、送信失敗、受信成功、受信失敗に関する指標の照会が可能になりました。
...
#### 機能改善・変更
* [Console/API]国際送信の受信結果がDLR結果を反映するように改善されました。
    * 受信成功はDLR状態がDELIVERED状態であることを意味します。
    * 受信失敗は、DLRの状態がEXPIRED、FAILED、REJECTED、UNKNOWNの状態であることを意味します。
    * 国際送信結果が受信失敗の場合、受信結果コードは次の文書を参照してください。
        * [[受信結果コード](./error-code/#_1)]
* [Console]国際送信統計改善
    * 国際送信統計分類にメッセージタイプ照会フィルタを追加しました。
    * 国際送信統計分類に受信イベントが追加されました。
    * 統計イベントの詳細については、 [[統計 > 統計照会](./console-guide/#_42)]を参照してください。
* [API]国際送信統計API改善
    * 国際送信統計APIに受信イベントを追加しました。
    * 詳細については、 [[API v3.0ガイド > 統計検索 - 国際送信](./api-guide/#-_2)]を参照してください。

<a id="added-features-2"></a>

#### 機能追加
* [Console/API]送信リスト照会時に国内/国際区分フィルタ、国コードフィルタ追加
    * SMSリクエスト別照会、大量SMS送信照会、タグSMS送信照会コンソール画面に国内/国際区分フィルタ、国コードフィルタを追加しました。
    * 全てのバージョンのSMS送信リスト検索、長文MMS送信リスト検索、予約送信リスト検索、認証用SMS送信リスト検索、大量送信受信者リスト検索、タグ送信受信者リスト検索APIのQuery ParameterにreceiverRegion、countryCodeフィールドを追加しました。
    * 詳細については、[[API v3.0ガイド](./api-guide)], [[API v2.4ガイド](./api-guide-v2.4)], [[API v2.3ガイド](./api-guide-v2.3)], [[API v2.2ガイド](./api-guide-v2.2)]を参照してください。

<a id="bug-fixes-2"></a>

#### バグ修正
* [API]国際送信統計APIバグ修正
    * 国際送信統計APIでコンバージョン率収集リクエスト送信成功数、コンバージョン数イベントが表示されないバグを修正しました。
...
#### 機能追加
* [ETC] 転換率ベースのブロックが発生した場合、CloudTrailログを追加
...
#### 機能追加
* [API]メッセージ送信結果アップデートWebフックにメッセージ件数フィールドを追加
    * メッセージ結果アップデートWebフックに送信されたメッセージ件数を表すmessageCountフィールドが追加されました。
    * 詳細については、 [[Webフックガイド > イベントタイプ別hooks定義 > メッセージ送信結果コードアップデート](./webhook/#_4)]を参照してください。
* [API]国際送信DLRアップデートWebフック追加
    * 国際送信DLRアップデートWebフックが追加されました。
    * 詳細については、 [[Webフックガイド > イベントタイプ別hooks定義 > 国際送信DLRアップデート](./webhook/#dlr)]を参照してください。
...
#### 機能追加
* [Console/API]組織別月間送信量制限機能を追加
    * 組織別月別送信量制限機能を追加しました。
    * 詳細については、 [[SMS > サービスポリシー > 送信ポリシー > 月別送信量制限](./sending-policy/#_10)]を参照してください。
...
#### 機能追加
* [Console/API]国際送信統計分類に統計イベントを追加
    * 国際送信統計分類にコンバージョン待機(READY)、コンバージョン完了(CONVERTED)統計イベントが追加されました。
    * 詳細については、 [[APIガイド > 統計検索 - 国際送信](./api-guide/#-_2)]を参照してください。
* [Console]国際SMS送信のコンバージョン率に基づくブロックルール設定機能を追加
    * 国際SMS送信のコンバージョン率に基づくブロックルールを設定できる機能が追加されました。
        * コンバージョン率ブロックしきい値、最小ブロック件数、コンバージョン率計算時間の設定が可能です。
    * 詳細については、 [[コンソール使用ガイド > 国際SMS送信設定](./console-guide/#sms_8)]を参照してください。
<a id="feature-updates-4"></a>

#### 機能改善
* [ETC]国際SMS送信コンバージョン率に基づくブロック解除時のコンバージョン率計算ルール変更
    * コンバージョン率に基づくブロック国解除後、変換送信時に再ブロックが発生しないようにコンバージョン率の計算ルールが変更されました。
    * 詳細については、 [[国際SMS送信ポリシー > 国際SMSコンバージョン率に基づく送信ブロック](./international-sending-policy/#sms_2)]を参照してください。 
...
#### 機能改善・変更
* [ETC]国際送信サポート国コード追加及び国名の変更
    * プエルトリコの国コード「+1939」が送信可能国に追加されました。
    * マヨットの国名を「マヨート/コモロ」に変更しました。
    * オランダ領アンティル諸島の国名を「オランダ領アンティル/キュラソー」に変更しました。
...
* [Console/API]国際SMS送信コンバージョン機能追加
    * 送信API変更
        * v3.0 SMS送信、 v3.0認証SMS送信、 v3.0広告SMS送信時にコンバージョン率収集リクエストを表すuseConversionフィールドが追加されました。
        * このフィールドを利用したコンバージョン率収集リクエストは、国際送信リクエストに対してのみ有効です。
    * コンバージョンAPIを追加
        * コンバージョン率収集リクエストに対してコンバージョンを試みるAPIが追加されました。
    * サービス設定 > 国際SMS送信設定タブにコンバージョン率によるブロックと通知設定を追加しました。
        * 国際SMS送信設定画面にコンバージョン率に基づく送信ブロックと通知機能が追加されました。
        * コンバージョン率に基づく送信ブロックを適用するブロック国設定機能が追加されました。
    * 詳細については、 [[国際SMS送信ポリシー > 国際SMSコンバージョン率に基づく送信ブロック](./international-sending-policy/#sms_2)]を参照してください。
* [Console]コンバージョン率に基づく送信ブロックのWebフックを追加
    * コンバージョン率に基づく送信ブロックが発生した場合、Webフックを受け取ることができます。
    * Webフックは特定国家送信信がコンバージョン率によってブロックされた場合に発生します。
...
* [Console]ロールの細分化
    * ロールに基づいてSMSメニューへのアクセスと機能制御権限が別途付与されるように機能が追加されました。
    ([リンクします](https://docs.nhncloud.com/ko/nhncloud/ko/console-guide/#_24))
...
---
* [Console] 大量送信時にアップロードする受信者ファイルのサイズ制限を引き上げ
...
---
* [Console] 大量送信時にテンプレートパラメータに4バイト絵文字が含まれている場合、処理中のままになるバグを修正
    * テンプレートパラメータに4バイト絵文字が含まれている場合、無効な受信者としてフィルタリングされるように修正しました。

<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (no corresponding heading in ko outline) -->
#### 機能改善・変更
...
<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (duplicate of same date in ko but different L3 context) -->
### 2019.12.24
...
* [API] 発信番号認証リクエスト履歴照会条件に発信番号追加
    ...
    * 詳細については、[[API ガイド](./api-guide/#api_1)] を参照してください。
...
* [Console/API] テンプレートID長の増加
    * テンプレートIDの長さ制限が、既存の10文字から50文字に変更されました。
...
* [Console] ダウンロードした CSV ファイルを Excel で開く場合の韓国語文字化け現象を解決
    * Excel で CSV ファイルを開いても韓国語が文字化けしないよう BOM タイプを追加しました。
* [API] 結果更新基準メッセージ照会 API フィールド追加
    * 結果更新基準メッセージ照会 API フィールドに通信会社コードが追加されました。
    * 詳細については「[API ガイド](./api-guide/#_28)」を参照してください。

<a id="december-27-2018"></a>

### 2018.12.27

<!-- pre-align: ko에 대응 섹션 없음 — 검토 필요 (target 機能改善 but ko has different context/position) -->
...
---
* [API] 予約送信一覧照会/キャンセル API 追加
    * 予約送信一覧を照会してキャンセルできる API が追加されました。
    * 詳細については、[[API ガイド](./api-guide/#_73)] を参照してください。
