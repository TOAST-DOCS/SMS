...
#### 기능 개선/변경
* [Console] SMS 서비스 비활성화 시 잔여 리소스 경고창 안내
    * 080 수신 거부 서비스를 사용중인 경우, 080 수신 거부 서비스 해지 이후 SMS 서비스 비활성화가 가능합니다.
* [Console] 예약 발송 일괄 취소 기능 추가
    * 예약 발송 건 중 예약 대기 상태인 메시지에 한하여 발송 취소가 가능합니다.
* [Console/API] 광고 메시지 발송 시 080 수신 거부 번호 상태가 사용중인 경우에만 발송 가능하도록 개선
    * 080 수신 거부 번호 상태가 '사용중'인 상태일 때만 발송이 가능합니다.
* [API] 인증용 SMS 발송 시 인증 문구 유효성 검사 강화 
    * 인증용 SMS 발송 시 인증 문구가 포함되어 있지 않은 경우 메시지 발송이 불가능합니다.
    * 자세한 사항은 [[API 가이드](./api-guide/#precautions-authword)] 참고하시기 바랍니다.

#### 버그 수정
* [API] 발신 번호 인증 요청 내역 조회 시 comment 필드가 정상적으로 노출되지 않은 문제 수정
